...
function getLocaleLanguage$1() {
  let localeLanguage = "";
  {
    localeLanguage = normalizeLocale(swan.getSystemInfoSync().language) || LOCALE_EN;
  }
...
function initWrapper(protocols2) {
  ...
  return function wrapper(methodName, method) {
    ...
    return function(arg1, arg2) {
      ...
      const returnValue2 = swan[options.name || methodName].apply(swan, args);
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__C4E911F",
    appName: "minprogram_share",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
const offError = {
  args(fromArgs) {
    ...
    if (!app.$vm) {
      ...
      const index2 = swan.$onErrorHandlers.findIndex((fn) => fn === fromArgs);
      if (index2 !== -1) {
        swan.$onErrorHandlers.splice(index2, 1);
      }
...
function initUni(api, protocols2, platform = swan) {
...
function initRuntimeSocketService() {
  const hosts = "172.16.40.190,127.0.0.1";
  const port = "8090";
  const id2 = "mp-baidu_ACWSEt";
...
function callHook(name, args) {
  ...
  {
    if (name === "onLoad" && args && args.__id__ && isFunction(swan.getEventChannel)) {
...
function parsePropType(type, defaultValue) {
  ...
  }
  {
    if (
      ...
      defaultValue === false && isArray(type) && type.length === 2 && type.indexOf(String) !== -1 && type.indexOf(Boolean) !== -1
    ) {
      return Boolean;
    }
  }
